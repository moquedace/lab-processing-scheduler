...
accent_bar <- image_blank(8L, 340L, color = col_accent)
img <- image_composite(img, accent_bar, offset = "+64+132")

# ── logo GeoCiS ──────────────────────────────────────────────────────────────
if (file.exists(logo_path)) {
  logo <- image_read(logo_path) |>
    image_resize("108x108") |>
    image_background("none")
  img <- image_composite(img, logo, offset = "+64+48")
} else {
...
img <- image_annotate(
  img,
  text     = "RESERVA DOS COMPUTADORES DE PROCESSAMENTO",
  gravity  = "NorthWest",
  location = "+84+178",
  color    = col_title,
  size     = 68L,
...
img <- image_annotate(
  img,
  text     = "Sistema de agendamento das estaçoes de alto desempenho do GeoCiS",
  gravity  = "NorthWest",
  location = "+84+282",
  color    = col_subtitle,
  size     = 34L,
...
# ── instituição (rodapé esquerdo) ─────────────────────────────────────────────
img <- image_annotate(
  img,
  text     = "GeoCiS  ·  Departamento de Ciencia do Solo  ·  ESALQ  ·  USP",
  gravity  = "SouthWest",
  location = "+84+50",
  color    = col_footer,
  size     = 28L,
